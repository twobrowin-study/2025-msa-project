...
```bash
docker build --platform linux/amd64,linux/arm64 . --push -t twobrowin/2025-msa-project:hw5.0.7
```
...
# Установка ingress
helm upgrade --install -n ingress-nginx ingress-nginx ingress-nginx/ingress-nginx -f ./infra-charts/ingress-values-dev.yaml
...
```bash
# Развёртывание БД
helm upgrade --install -n otus-2025-msa pg-dev oci://registry-1.docker.io/bitnamicharts/postgresql -f ./infra-charts/pg-values-dev.yaml

# Port-forward для подключения к БД
kubectl port-forward -n otus-2025-msa svc/pg-dev-postgresql 5432:5432
```
...
### Развёртывание Prometheus, Prometheus Operator и Grafana
...
# Создание пароля администратора grafana
kubectl create secret -n monitoring generic grafana-dev --from-literal='admin-user=<ADMIN_USER>' --from-literal='admin-password=<ADMIN_PASSWORD>'

# Развёртывание prometheus
helm upgrade --install -n monitoring prometheus-stack prometheus-community/kube-prometheus-stack -f infra-charts/prometheus-stack-values-dev.yaml
```

Для доступа к UI Prometheus следует сделать запрос на хост `http://prometheus.arch.homework/`

Для доступа к UI Grafana следует сделать запрос на хост `http://grafana.arch.homework/`